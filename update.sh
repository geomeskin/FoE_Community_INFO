#!/usr/bin/env bash
# =============================================================================
# update.sh  —  Build the FoE Community INFO pages and push to GitHub Pages
# =============================================================================
# Usage:
...
echo ""
echo "============================================"
echo "  FoE Community INFO — Update Workflow"
echo "============================================"
echo ""
# ── 1. Run the builder ────────────────────────────────────────────────────────
PYTHON=$(command -v py || command -v python3 || command -v python || echo "/c/Users/Alex/AppData/Local/Microsoft/WindowsApps/python3.11.exe")
if [ "$#" -gt 0 ]; then
    "$PYTHON" build.py "$@"
else
    "$PYTHON" build.py
...
if git diff --cached --quiet; then
    echo "  No changes detected — all pages are already up to date."
    echo "  Nothing to push."
else
    BUILD_DATE=$(date -u "+%Y-%m-%d %H:%M UTC")
    git commit -m "Update community INFO — ${BUILD_DATE}"
    echo ""
